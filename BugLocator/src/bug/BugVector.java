...
{
    private static final String HOME_FOLDER = Property.getInstance()
            .getWorkDir() + Property.getInstance().getSeparator();
    private static final String BUG_CORPUS_FOLDER = "BugCorpus" +
            Property.getInstance().getSeparator();

    public void create()
            throws WVToolException, IOException
    {
        WVTool wvt = new WVTool(false);
        WVTConfiguration config = new WVTConfiguration();
        final WVTStemmer porterStemmer = new PorterStemmerWrapper();
        config.setConfigurationRule("stemmer",
        ...
        config.setConfigurationRule("output",
                new WVTConfigurationFact(wvw));
        config.setConfigurationRule("vectorcreation",
