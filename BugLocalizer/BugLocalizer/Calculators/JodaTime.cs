    ...
    public class JodaTime : BaseExecutable
    {
        ...
        private static void RunTssOnDataset(string dataset)
        {
            ...
            for (int i = 0; i < totalbugsCount; i++)
            {
                ...
                try
                {
                    CodeFilesWithContent.Clear();
                    IdfDictionary.Clear();
                    TfDictionary.Clear();
                    TfIdfDictionary.Clear();
                    WordAndContainingFiles.Clear();
                    _uk = null;
                    _sk = null;
                    _vkTranspose = null;
                    ...
                    // Read all files
                    foreach (var file in new DirectoryInfo(bugFolderPath + CorpusWithFilterFolderName).GetFiles())
                    {
